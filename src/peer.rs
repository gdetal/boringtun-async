...
/// Represents a peer in Wireguard.
///
/// Peers are responsible for encapsulating and decapsulating packets,
/// managing endpoints, and updating timers.
#[pin_project]
pub struct Peer {
    tunnel: Tunn,
    endpoint: Endpoint,
...
impl Peer {
    ...
    /// Sets the endpoint for the peer.
    /// this shuts down the current endpoint if it is different from the new one.
    ...
    /// Shuts down the current endpoint.
    pub(crate) fn shutdown_endpoint(&mut self) {
        self.endpoint.sock.take();
    }

    /// Connects the endpoint to the peer.
    /// The peer will use connected UDP sockets for sending packets.
    ...
    /// Encapsulates a packet and sends it to the peer.
    ...
    /// Decapsulates a packet and returns the decapsulated packet.
    pub(crate) fn decapsulate<'a>(
        ...
        {
            TunnResult::Done => {}
            TunnResult::Err(e) => log::debug!("decapsulate error {:?}", e),
            TunnResult::WriteToNetwork(packet) => {
                flush = true;
        ...
        if flush {
            // Flush pending queue of packets
    ...
    /// Updates the timers for the peer.
    fn update_timers(&mut self) {
        match self.tunnel.update_timers(&mut self.buffer) {
            ...
            TunnResult::Err(e) => {
                log::debug!("update_timers error: {e:?}")
            }
            TunnResult::WriteToNetwork(packet) => {
                self.endpoint.try_send(packet).ok();
            }
    ...
    /// Poll for new packets on the connected socket.
    fn poll_recv(&mut self, cx: &mut Context<'_>, buf: &mut BytesMut) -> Poll<usize> {
        ...
        if let Some(ref conn) = self.endpoint.sock {
            return match conn.poll_recv(cx, &mut read_buf) {
                ...
                Poll::Ready(Err(e)) => {
                    log::debug!("unable to receive packets: {e}");
                    self.shutdown_endpoint();
                    Poll::Pending
                }
...
/// Represents a socket that listens only on the wireguard port.
/// This socket is used for receiving packets from any peer when
/// the connected sockets are not yet established.
#[pin_project]
...
impl Stream for PeerSocketInner {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        // Parse and rate-limit the packet
        let parsed_packet =
        ...
        // Parse the packet content and return the peer
        ...
        // Return the parsed packet and the peer associated with it
        Poll::Ready(Some(Ok((peer, dst_buf_mut.split_to(len), addr))))
    }
}

/// Represents a socket that listens on both IPv4 and IPv6.
#[pin_project]
...
impl Stream for PeerSocket {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        // Swap the order of the IP socket to round-robin between them
